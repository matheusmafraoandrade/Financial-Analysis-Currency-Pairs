...
unit_root <- fUnitRoots::adfTest(df$jpy, lags = 1, type = c("nc"))
print(unit_root)
...
acf_arma <- stats::acf(df$jpy, na.action = na.pass, plot = FALSE, lag.max = 15)

# Gráfico da Função de Autocorrelação. 
plot(acf_arma, main = "", ylab = "", xlab = "Defasagem")
title("Função de Autocorrelação (FAC) do JPY/BRL", adj = 0.5, line = 1)

# Função de Autocorrelação parcial
pacf_arma <- stats::pacf(df$jpy, na.action = na.pass, plot = FALSE, lag.max = 15)

# Gráfico da Função de Autocorrelação parcial. 
plot(pacf_arma, main = "", ylab = "", xlab = "Defasagem")
title("Função de Autocorrelação Parcial (FACP) do JPY/BRL", adj = 0.5, line = 1)

# Teste Ljung-Box
Box.test(df$jpy,type="Ljung-Box",lag=1)
...
for (i in 1:nrow(pars)) {
  modelo[[i]] <- arima(df$jpy, order = unlist(pars[i, 1:3]), method = "ML")
}
...
tamanho_amostra <- rep(length(df$jpy), length(modelo))
...
# Planilha
write_xlsx(resultado_arma,"C:\\Users\\matma\\OneDrive\\TCC\\R\\aic_bic_jpy.xlsx")


# Como resultado temos que o modelo escolhido pelo BIC é o ARMA(1,0)
media_condicional <- arima(df$jpy, order = c(1,0,0), method = "ML")
...
# Train Test Split
train = head(df$jpy, 0.50*length(df$jpy))
test = tail(df$jpy, 0.50*length(df$jpy))
...
# Planilha
write_xlsx(resultado_arma_garch,"C:\\Users\\matma\\OneDrive\\TCC\\R\\garch_aic_bic_jpy.xlsx")
...
#media_variancia_condicional <- fGarch::garchFit(~arma(0,0)+garch(1,1), data = dataset, trace = FALSE, 
...
gm = ugarchfit(data = df$jpy, spec = spec, out.sample = length(tail(df$jpy, 0.18*length(df$jpy))))
show(gm)

jarque.bera.test(na.remove(residuals(gm, standardize=TRUE)))

pred = ugarchforecast(gm, data = df$jpy, n.ahead = 30, n.roll = 1, out.sample = length(tail(df$jpy, 0.18*length(df$jpy))))
pred
plot(pred)
...
upp = mu + 2 * v4
low = mu - 2 * v4
tdx = c(1:length(df$jpy)) / 12 + 1973
plot (tdx, df$jpy, xlab = 'year', ylab = 'series', type = 'l', ylim = c(-0.6, 0.6))
...
#  - H0: resíduos são não autocorrelacionados
#  - H1: resíduos são autocorrelacionados
acf_est <- stats::acf(residuals(gm, standardize=FALSE), na.action = na.pass, plot = FALSE, lag.max = 15)
plot(acf_est, main = "", ylab = "", xlab = "Defasagem")
title("Função de Autocorrelação (FAC) dos Resíduos", adj = 0.5, line = 1)
Box.test(residuals(gm, standardize=TRUE), type="Ljung", lag=9)
