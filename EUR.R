...
unit_root <- fUnitRoots::adfTest(df$eur, lags = 2, type = c("nc"))
print(unit_root)
...
acf_arma <- stats::acf(df$eur, na.action = na.pass, plot = FALSE, lag.max = 15)

# Gráfico da Função de Autocorrelação. 
plot(acf_arma, main = "", ylab = "", xlab = "Defasagem")
title("Função de Autocorrelação (FAC) do EUR/BRL", adj = 0.5, line = 1)

# Função de Autocorrelação parcial
pacf_arma <- stats::pacf(df$eur, na.action = na.pass, plot = FALSE, lag.max = 15)

# Gráfico da Função de Autocorrelação parcial. 
plot(pacf_arma, main = "", ylab = "", xlab = "Defasagem")
title("Função de Autocorrelação Parcial (FACP) do EUR/BRL", adj = 0.5, line = 1)

# Teste Ljung-Box
Box.test(df$eur,type="Ljung-Box",lag=1)
...
for (i in 1:nrow(pars)) {
  modelo[[i]] <- arima(df$eur, order = unlist(pars[i, 1:3]), method = "ML")
}
...
tamanho_amostra <- rep(length(df$eur), length(modelo))
...
# Planilha
write_xlsx(resultado_arma,"C:\\Users\\matma\\OneDrive\\TCC\\R\\aic_bic_eur.xlsx")


# Como resultado temos que o modelo escolhido pelo BIC é o ARMA(1,0)
media_condicional <- arima(df$eur, order = c(3,0,3), method = "ML")
...
acf_est <- stats::acf(modelo[[which.min(resultado_arma$aic)]]$residuals, na.action = na.pass, plot = FALSE, lag.max = 15)
plot(acf_est, main = "", ylab = "", xlab = "Defasagem")
title("Função de Autocorrelação (FAC) dos Resíduos", adj = 0.5, line = 1)
Box.test(modelo[[which.min(resultado_arma$aic)]]$residuals, type="Ljung", lag=1)
...
acf_square <- acf(modelo[[which.min(resultado_arma$aic)]]$residuals^2, na.action = na.pass, plot = FALSE, lag.max = 20)
plot(acf_square, main = "", ylab = "", xlab = "Defasagem")
title("FAC do quadrado dos resíduos do AR(1)", adj = 0.5, line = 1)
archTest(modelo[[which.min(resultado_arma$aic)]]$residuals)
...
jarque.bera.test(na.remove(modelo[[which.min(resultado_arma$aic)]]$residuals))
normalTest(na.remove(modelo[[which.min(resultado_arma$aic)]]$residuals), method = "jb")
...
acf_residuals <- acf(media_condicional$residuals^2, na.action = na.pass, plot = FALSE, lag.max = 20)
plot(acf_residuals, main = "", ylab = "", xlab = "Defasagem")
title("FAC do quadrado dos resíduos do ARMA(3,3)", adj = 0.5, line = 1)
...
title("FACP do quadrado dos resíduos do ARMA(3,3)", adj = 0.5, line = 1)
...
# Train Test Split
train = head(df$eur, 0.82*length(df$eur))
test = tail(df$eur, 0.18*length(df$eur))
...
# Planilha
write_xlsx(resultado_arma_garch,"C:\\Users\\matma\\OneDrive\\TCC\\R\\garch_aic_bic_eur.xlsx")
...
media_variancia_condicional <- fGarch::garchFit(~arma(1,1)+garch(1,1), data = dataset, trace = FALSE, 
...
spec = ugarchspec(mean.model=list(armaOrder=c(1,1)), distribution.model = "std",
                  variance.model=list(model= "sGARCH", garchOrder=c(2,1)), fixed.pars = list(mu=0, alpha2=0))
gm = ugarchfit(data = df$eur, spec = spec, out.sample = length(tail(df$eur, 0.15*length(df$eur))))
show(gm)

jarque.bera.test(na.remove(residuals(gm, standardize=TRUE)))

pred = ugarchforecast(gm, data = df$eur, n.ahead = 30, n.roll = 0, out.sample = length(tail(df$eur, 0.18*length(df$eur))))
pred
plot(pred)
...
upp = mu + 2 * v4
low = mu - 2 * v4
tdx = c(1:length(df$eur)) / 12 + 1973
plot (tdx, df$eur, xlab = 'year', ylab = 'series', type = 'l', ylim = c(-0.6, 0.6))
